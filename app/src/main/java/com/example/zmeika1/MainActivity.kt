...
class MainActivity : Activity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        newgame.setOnClickListener {
            ...
            fun checkFoodCollision() { // проверяет, произошло ли столкновение между головой змеи и едой
                val distanceThreshold = 50

                val distance = sqrt((snake.x - meat.x).pow(2) + (snake.y - meat.y).pow(2))

                if (distance < distanceThreshold) { // Проверка, не меньше ли расстояние между головой змеи и едой
            ...
            val runnable = object : Runnable {
                override fun run() {
                    ...
                    when (currentDirection) { //направление змеи
                        "up" -> {
                            snakeY -= 8
